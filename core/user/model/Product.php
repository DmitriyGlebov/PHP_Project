...
class Product extends BaseModel
{
    ...
    public function getProduct($id = NULL){
        ...
        $stmt = $this->db->prepare("SELECT p.id, p.name, p.code, p.description, c.name as category_name FROM products as p
    ...
    public function updateProduct($data, $messages){

        $stmt = $this->db->prepare("UPDATE products SET `name` = ? , `code` = ?, `description` = ?, `category_id` = ? WHERE id = ? ");

        if($stmt->execute([$data['prodName'], $data['code'], $data['description'], $data['category_id'], $data['id']])){
