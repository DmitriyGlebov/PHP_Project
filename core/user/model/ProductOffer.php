...
class productOffer extends Model
{
    ...
    public function getProductOffers($product_id = NULL, $arrLimit = []){
        ...
        $stmt = $this->db->prepare("SELECT po.id, po.count, po.price, po.image, p.name as product_name FROM product_offers as po
                                    JOIN products as p 
    ...
    public function getProductOffer($id){
        ...
        $stmt = $this->db->prepare("SELECT pO.id, pO.count, pO.price, pO.product_id, pO.image, p.name as product_name FROM product_offers as pO
                                    JOIN products as p
    ...
    public function updateProductOffer($data, $messages, $image = NULL){

        $stmt = $this->db->prepare("UPDATE product_offers SET `product_id` = ? , `count` = ?, `price` = ?, `image` = ? WHERE id = ? ");

        if($stmt->execute([$data['product_id'], $data['price'], $data['count'], $image, $data['product_offer_id']])){
    ...
    public function getProductsFromOrder($order_id){
        ...
        $stmt = $this->db->prepare("SELECT p.id as product_id, p.name, p.image, po.id, po.price, op.countInOrder FROM products as p
